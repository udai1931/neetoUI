...
Let's say that the project name is `neetoUI`. Execute the command below to
replace all occurrences of `neetoUI` with `neetoUI`.

```bash
perl -pi -w -e 's/neetoUI/neetoUI/g;' $(git ls-files)
perl -pi -w -e 's/neetoUI/neetoUI/g;' $(git ls-files)
```
